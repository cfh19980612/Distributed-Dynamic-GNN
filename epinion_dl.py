...
class epinion_dataset():
    def __init__(self,args, rankID):
        assert args.task in ['link_pred'], 'epinion only implements link_pred'  # assert:断言， 当条件不满足直接返回异常
        ...
        args.epinions_args = u.Namespace(args.epinions_args)
        ...
        '''
        edges = self.load_edges(args.epinions_args, rankID)  # 从文件中按行读取数据，构造成tensor矩阵
        # timesteps = u.aggregate_by_time(edges[:,self.ecols.TimeStep], args.epinions_args.aggr_time)  # 首先取edges矩阵的第四列(对应时刻)所有元素
        ...
        self.feats_per_node = args.epinions_args.feats_per_node  # 每个点的特征个数
    ...
    def load_edges(self, epinions_args, rankID, starting_line = 1):
        file = os.path.join(epinions_args.folder,epinions_args.edges_file)
        with open(file) as f:
