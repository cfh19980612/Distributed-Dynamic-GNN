...
def get_1_hot_deg_feats(adj,max_deg,num_nodes):
    #For now it'll just return a 2-hot vector
    new_vals = torch.ones(adj['idx'].size(0))  # 有连接边的点赋值为1
    new_adj = {'idx':adj['idx'], 'vals': new_vals}
    degs_out, _ = get_degree_vects(new_adj,num_nodes)  #生成度向量N*1
...
def get_sp_adj(edges,time,weighted,time_window):
    idx = edges['idx']  # edges为字典，其中idx为tensor矩阵，保存起点和终点id以及时刻， vals保存tensor张量，记录边的权重

    # 获得mask，用来索引时刻<time,并且在【time-window，time】区间的边(edges['idx'])
    subset = idx[:,ECOLS.time] <= time  # 取出<= time的时刻的tensor mask, 返回值为tensor的bool张量
...
def get_node_mask(cur_adj,num_nodes):
    mask = torch.zeros(num_nodes) - float("Inf")  # 0-正无穷
    # 所有边中出现的点对应的mask值变为0
